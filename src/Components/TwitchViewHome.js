...
const TwitchViewHome = props => {
    ...
    const [participantPoints, changeParticipantPoints] = useState([])
    const [finalScores, changeFinalScores] = useState([])
    const [startingScores,changeStartingScores]=useState([])
    ...
    useEffect(() => {
        const twitchStreamUpdate = async () => {
            ...
            let getfinalScores=await  window.contract.getArrayOfFinalScores({title:activeChallenges[activeChallengeIndex]})
            let getStartingStcoresList=await window.contract.getStartScores({title:activeChallenges[activeChallengeIndex]})
                console.log(getfinalScores)
                console.log(getStartingStcoresList)
            console.log(getfinalScores)
            changeFinalScores(getfinalScores)
            changeParticipantPoints(resolvedCurrentScores)
            changeGameParticipants(resolvedListOfParticipants[activeChallengeIndex])
            changeStartingScores(getStartingStcoresList)
                                ...
                                return (
                                    <tr key={index}>
                                        ...
                                        <td className="align-middle" style={{ backgroundColor: 'black', textAlign: 'center' }}>
                                            <ProgressBar now={Number((participantPoints[index])-startingScores[index])/(Number(finalScores[index])-startingScores[index])*100}></ProgressBar>
                                            {console.log(Number(finalScores[index]))}
                                        </td>
